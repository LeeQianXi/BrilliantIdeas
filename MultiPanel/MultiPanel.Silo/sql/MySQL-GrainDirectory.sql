...
-- Parse silo addresses into temporary table for batch operation
    ...
    WITH RECURSIVE SiloAddressesCTE AS
                       (SELECT SUBSTRING_INDEX(_SiloAddresses, '|', 1) AS Value,
                               SUBSTRING(_SiloAddresses, CHAR_LENGTH(SUBSTRING_INDEX(_SiloAddresses, '|', 1)) + 2,
                                         CHAR_LENGTH(_SiloAddresses))  AS Others,
                               1                                       AS Level
                        UNION ALL
                        SELECT SUBSTRING_INDEX(Others, '|', 1) AS Value,
                               SUBSTRING(Others, CHAR_LENGTH(SUBSTRING_INDEX(Others, '|', 1)) + 2,
                                         CHAR_LENGTH(Others))  AS Others,
                               Level + 1
