...
public static partial class ReferencePool
{
    ...
    public static void ClearAll()
    {
        lock (ReferenceCollections)
        {
            foreach (var referenceCollection in ReferenceCollections) referenceCollection.Value.RemoveAll();
    ...
    {
        if (reference == null) throw new NetUtilityException("Reference is invalid.");
    ...
    private static void InternalCheckReferenceType(Type referenceType)
    {
        ...
        if (referenceType == null) throw new NetUtilityException("Reference type is invalid.");

        if (!referenceType.IsClass || referenceType.IsAbstract)
            throw new NetUtilityException("Reference type is not a non-abstract class type.");

        if (!typeof(IReference).IsAssignableFrom(referenceType))
            throw new NetUtilityException(Utility.Text.Format("Reference type '{0}' is invalid.",
                referenceType.FullName));
    }

    private static ReferenceCollection GetReferenceCollection(Type referenceType)
    {
        if (referenceType == null) throw new NetUtilityException("ReferenceType is invalid.");

        ReferenceCollection? referenceCollection;
        lock (ReferenceCollections)
        {
            if (ReferenceCollections.TryGetValue(referenceType, out referenceCollection)) return referenceCollection;
